...
for (var i = 0; i < restrictedWords.length; i++) {
  let loopedWords = restrictedWords[i].replaceAll(
    restrictedWords[i],
    replaceWords[i]
  );
...
function newArrayOne(comments) {
  for (var i = 0; i < comments.length; i++) {
    let result = comments[i];
    let words = result.split(" ");
    for (var j = 0; j < words.length; j++) {
      let wordResult = words[j];
      switch (wordResult.toLowerCase()) {
        case "dude": // not been found, why?
          wordResult = "Friendo";
          break;
        case "darn":
          wordResult = "Heckn";
...
//     change += Array1[i].replace("Dude", "Friendo");
//     console.log(change);
//   }
// }
...
//   let myPosition = Array1.indexOf("Assassin");
//   console.log("Position of Assassin: " + myPosition);
// }
// newReplacement();
