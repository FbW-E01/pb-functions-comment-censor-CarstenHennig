...
// Below there are three arrays of comments. Go through each comment in each array of those comments, printing out each comment - but replace the banned words with good words. Important: replace both uppercase and lowercase versions,
...
//newsEE = oldEE.replace("c", "*");

console.log(newEE);
function replaceOne(a, b) {
  let newEE = oldEE.replace("e", "*");
  let newEE = oldEE.replace("c", "*");
}
